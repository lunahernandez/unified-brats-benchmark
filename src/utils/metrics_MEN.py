...
# https://github.com/rachitsaluja/BraTS-2024-Metrics/blob/main/metrics_MEN.py
...
def compute_hd95(gt_mask, pred_mask, spacing):
    ...
    Returns:
        HD95 value. Returns 0.0 if both masks are empty and np.inf if only
        one mask is empty.
    """
    ...
    if np.all(gt_mask == 0) or np.all(pred_mask == 0):
        return np.inf
    ...
    return surface_distance.compute_robust_hausdorff(
        surface_distances,
        95,
    )
...
def get_TissueWiseSeg(prediction_matrix, gt_matrix, tissue_type):
    """Convert segmentations to binary masks for a BraTS tissue or region.

    Args:
        prediction_matrix: Predicted segmentation label map.
        gt_matrix: Ground-truth segmentation label map.
        tissue_type: Tissue or region to isolate. Supported values are
            "WT", "TC", "NETC", "SNFH", and "ET".

    Returns:
        A tuple with the binary prediction and ground-truth masks.
    """
    ...
    if tissue_type == "WT":
        ...
        raise ValueError(
            f"Unsupported tissue type: {tissue_type}. "
            "Use WT, TC, NETC, SNFH, or ET."
        )
...
def get_LesionWiseResults(pred_file, gt_file, challenge_name, output=None):
    ...
    label_values = ["WT", "TC", "NETC", "SNFH", "ET"]
