...
# Modified for this project by Luna Hernández and Diego Torrejón, 2026.
...
def iou(im1, im2):
    """
    Computes Intersection over Union for two binary images.
    ...
    Output
    ======
    iou_score: IoU score between two images
    """
...
def save_tmp_files(pred_file, gt_file, dil_factor):
    ...
    tissue_list = ["WT", "TC", "NETC", "SNFH", "ET", "RC"]
...
def combine_lesions_NETC(netc_cc, et_cc, rc_cc):
    op_path = os.path.join(
        os.path.split(netc_cc)[0],
        os.path.split(netc_cc)[1].split(".")[0] + "_combined.nii.gz"
    )

    touching_labels = get_touching_labels(netc_cc, et_cc)
    if len(touching_labels) > 1:
        relabel_nifti_image(touching_labels,
                            nifti_image_path=netc_cc,
                            output_path=op_path)
    else:
        shutil.copy(netc_cc, op_path)

    touching_labels = get_touching_labels(op_path, rc_cc)
    if len(touching_labels) > 1:
        relabel_nifti_image(touching_labels,
                            nifti_image_path=op_path,
                            output_path=op_path)


def combine_lesions_SNFH(snfh_cc, et_cc, rc_cc, netc_cc):
    op_path = os.path.join(
        os.path.split(snfh_cc)[0],
        os.path.split(snfh_cc)[1].split(".")[0] + "_combined.nii.gz"
    )

    touching_labels = get_touching_labels(snfh_cc, et_cc)

    if len(touching_labels) > 1:
        relabel_nifti_image(touching_labels,
                            nifti_image_path=snfh_cc,
                            output_path=op_path)
    else:
        shutil.copy(snfh_cc, op_path)
    ...
    touching_labels = get_touching_labels(op_path, netc_cc)
    if len(touching_labels) > 1:
        relabel_nifti_image(touching_labels,
                            nifti_image_path=op_path,
                            output_path=op_path)
...
def combine_lesions_ET(et_cc, netc_cc, rc_cc):
    op_path = os.path.join(
...
def combine_lesions_RC(rc_cc, et_cc, netc_cc):
    op_path = os.path.join(
        os.path.split(rc_cc)[0],
        os.path.split(rc_cc)[1].split(".")[0] + "_combined.nii.gz"
    )

    touching_labels = get_touching_labels(rc_cc, et_cc)
    if len(touching_labels) > 1:
        relabel_nifti_image(touching_labels,
                            nifti_image_path=rc_cc,
                            output_path=op_path)

    else:
        shutil.copy(rc_cc, op_path)
...
def combine_lesions_tissues(netc_cc, snfh_cc, et_cc, rc_cc):

    for i in range(2):
        combine_lesions_NETC(netc_cc, et_cc, rc_cc)
        combine_lesions_ET(et_cc, netc_cc, rc_cc)
        combine_lesions_RC(rc_cc, et_cc, netc_cc)
        combine_lesions_SNFH(snfh_cc, et_cc, rc_cc, netc_cc)
...
def get_LesionWiseScores(prediction_seg, gt_seg, label_value, dil_factor):
    ...
    if (label_value == "SNFH") | (label_value == "NETC") | (label_value == "RC"):
        dil_factor = 5
    ...
    if np.all(gt_mat == 0) and np.all(pred_mat == 0):
        ...
        full_sd = surface_distance.compute_surface_distances(gt_mat.astype(bool),
                                                             pred_mat.astype(
                                                                 bool),
                                                             (sx, sy, sz))
    ...
    for gtcomp in range(np.max(gt_label_cc)):
        ...
        iou_score = iou(pred_tmp, gt_tmp)
        surface_distances = surface_distance.compute_surface_distances(
            gt_tmp.astype(bool),
            pred_tmp.astype(bool),
            (sx, sy, sz),
        )
        ...
        metric_pairs.append((intersecting_cc,
                            gtcomp, gt_vol, dice_score, iou_score, hd, nsd_05, nsd_10))
    ...
    return tp, fn, fp, gt_tp, metric_pairs, full_dice, full_iou, full_hd95, full_nsd_05, full_nsd_10, full_gt_vol, full_pred_vol, full_sens, full_specs
...
def get_LesionWiseResults(pred_file, gt_file, challenge_name, output=None):
    ...
    label_values = ['WT', 'TC', 'NETC', 'SNFH', 'ET', 'RC']
    ...
    rc_cc = f"./tmp_gt/{gt_file_name}/{gt_file_name}_RC_cc.nii.gz"
    snfh_cc = f"./tmp_gt/{gt_file_name}/{gt_file_name}_SNFH_cc.nii.gz"

    combine_lesions_tissues(netc_cc, snfh_cc, et_cc, rc_cc)
    ...
    rc_cc = f"./tmp_pred/{pred_file_name}/{pred_file_name}_RC_cc.nii.gz"
    snfh_cc = f"./tmp_pred/{pred_file_name}/{pred_file_name}_SNFH_cc.nii.gz"
    ...
    for l in range(len(label_values)):
        ...
        tp, fn, fp, gt_tp, metric_pairs, full_dice, full_iou, full_hd95, full_nsd_05, full_nsd_10, full_gt_vol, full_pred_vol, full_sens, full_specs = get_LesionWiseScores(
        ...
        metric_df = pd.DataFrame(
            metric_pairs, columns=['predicted_lesion_numbers', 'gt_lesion_numbers',
                                   'gt_lesion_vol', 'dice_lesionwise', 'iou_lesionwise', 'hd95_lesionwise', 'nsd05_lesionwise', 'nsd10_lesionwise']
        ...
        lesionwise_denominator = len(metric_df_thresh) + len(fp)

        if lesionwise_denominator == 0:
            lesion_wise_dice = np.nan
            lesion_wise_iou = np.nan
            lesion_wise_nsd_05 = np.nan
            lesion_wise_nsd_10 = np.nan
            lesion_wise_hd95 = np.nan
        else:
            lesion_wise_dice = (
                np.sum(metric_df_thresh['dice_lesionwise'])
                / lesionwise_denominator
            )
            lesion_wise_iou = (
                np.sum(metric_df_thresh['iou_lesionwise'])
                / lesionwise_denominator
            )
            lesion_wise_nsd_05 = (
                np.sum(metric_df_thresh['nsd05_lesionwise'])
                / lesionwise_denominator
            )
            lesion_wise_nsd_10 = (
                np.sum(metric_df_thresh['nsd10_lesionwise'])
                / lesionwise_denominator
            )
            lesion_wise_hd95 = (
                np.sum(metric_df_thresh['hd95_lesionwise'])
                + len(fp) * 337
            ) / lesionwise_denominator
        ...
        metrics_dict = {
            ...
            'LesionWise_Score_IoU': lesion_wise_iou,
    ...
    results_df.replace(np.inf, 337, inplace=True)
